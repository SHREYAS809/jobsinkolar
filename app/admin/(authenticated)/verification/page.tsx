                ...
                {QUEUE.map((item) => (
                    <div key={item.id} className="glass-panel p-6 md:p-10 rounded-[3rem] border border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-8 group hover:bg-white/5 transition-all">
                        <div className="flex gap-6 items-center">
                            <div className={`h-16 w-16 rounded-[1.5rem] flex items-center justify-center border border-white/10 shadow-inner ${item.type === "Employer" ? "bg-indigo-600/10 text-indigo-400" : "bg-blue-600/10 text-blue-400"
                                }`}>
                            ...
                            <div>
                                <div className="flex items-center gap-3 mb-1">
                                    ...
                                    <span className={`text-[8px] font-black uppercase tracking-[0.2em] px-2 py-0.5 rounded border ${item.priority === "High" ? "bg-rose-500/10 text-rose-400 border-rose-500/20" : "bg-white/5 text-slate-500 border-white/10"
                                        }`}>{item.priority}</span>
                                </div>
                                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                                    {item.type} Submission Protocol • <span className="text-slate-400 italic">Syncing {item.date}</span>
                                </p>
                            </div>
                        </div>

                        <div className="flex gap-4">
                            <button className="h-14 w-14 bg-white/5 rounded-2xl border border-white/10 flex items-center justify-center text-slate-400 hover:text-white transition-all">
                                <Eye size={20} />
                            </button>
                            <button className="h-14 px-6 bg-emerald-600 hover:bg-emerald-500 text-white font-black uppercase tracking-widest text-[10px] rounded-2xl flex items-center gap-3 shadow-lg shadow-emerald-500/10 active:scale-95 transition-all">
                                <CheckCircle2 size={18} /> Approve
                            </button>
                            <button className="h-14 px-6 bg-white/5 hover:bg-rose-600/10 text-slate-400 hover:text-rose-400 font-black uppercase tracking-widest text-[10px] border border-white/10 rounded-2xl flex items-center gap-3 active:scale-95 transition-all">
