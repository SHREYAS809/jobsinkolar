            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-1 space-y-8">
                    <div className="glass-panel p-10 rounded-[3rem] border border-white/5 bg-gradient-to-br from-indigo-600/20 to-purple-600/20 relative overflow-hidden group">
                        <div className="relative z-10">
                            <h3 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] mb-10">Credit Allocation</h3>
                            <p className="text-4xl font-black text-white mb-2">1,240 <span className="text-xs text-slate-500 tracking-normal">Credits</span></p>
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-10">Sufficient for 24 elite syncs</p>
                            <Button className="w-full bg-white text-slate-950 font-black uppercase tracking-widest text-[9px] h-12 rounded-xl">Recalibrate Balance</Button>
                        </div>
                    ...
                    <div className="glass-panel p-8 rounded-[3rem] border border-white/5 space-y-6">
                        <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest border-b border-white/5 pb-4">Protocol Plan</h4>
                        <div className="flex justify-between items-center">
                            <span className="text-sm font-black text-white uppercase tracking-tight">Enterprise Elite</span>
                            <span className="text-[10px] font-black text-emerald-400 bg-emerald-500/10 px-2 py-1 rounded border border-emerald-500/20">Active</span>
                        </div>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-loose">Next auto-sync on Nov 15, 2023</p>
                        <Button variant="ghost" className="w-full text-[9px] font-black text-white hover:bg-white/5 border border-white/10 rounded-xl h-10">Modify Protocol</Button>
                    </div>
                </div>
                                    ...
                                    {TRANSACTIONS.map((t) => (
                                        <tr key={t.id} className="hover:bg-white/5 transition-colors group">
                                            ...
                                            <td className="px-8 py-6 text-right">
                                                <button className="h-10 w-10 bg-white/5 rounded-xl border border-white/10 flex items-center justify-center text-slate-500 hover:text-white transition-all">
                                                    <Download size={16} />
                                                </button>
                                            </td>
                                        </tr>
